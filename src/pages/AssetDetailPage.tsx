...
export function AssetDetailPage() {
  ...
  const { asset, costs, revenues } = result.snapshot; const values = calculateAssetCosts(asset, costs, revenues, today);
  ...
  return <section><div className="page-heading"><h1>{asset.name}</h1><Link className="button" to={`/assets/${asset.id}/edit`}>编辑</Link></div>
    <div className="hero-metric"><small>{asset.costMode === 'day' ? '当前日均成本' : '当前次均成本'}</small><strong>{asset.costMode === 'day' ? dayMetric : useMetric}</strong>{asset.costMode === 'use' && <span>累计 {asset.usageCount} 次 · 辅助日均 {dayMetric}</span>}<span>持有 {values.daysOwned} 天</span>
      {asset.costMode === 'use' && <div className="usage-controls"><button className="primary" type="button" disabled={incrementing || atUsageLimit} onClick={handleIncrement}>{incrementing ? '记录中…' : '+ 使用一次'}</button><button type="button" disabled={incrementing || correctionBusy} onClick={openCorrection}>更正次数</button><small aria-live="polite">{atUsageLimit ? '使用次数已达到上限' : usageFeedback}</small></div>}
    </div>
    ...
    {values.netCostCents < 0 && <p className="notice">收益已超过投入</p>}{values.clockBeforePurchase && <p className="notice">设备日期早于购买日期，持有天数暂按 1 天计算。</p>}
    ...
    <h2>资产信息</h2><dl><dt>购买日期</dt><dd>{asset.purchaseDate}</dd><dt>观察方式</dt><dd>{asset.costMode === 'day' ? '按天' : '按次'}</dd><dt>到期日期</dt><dd>{asset.expiryDate ?? '未设置'}</dd><dt>到期状态</dt><dd>{expiryText(asset.expiryDate, today)}</dd>{asset.note && <><dt>备注</dt><dd>{asset.note}</dd></>}</dl>
