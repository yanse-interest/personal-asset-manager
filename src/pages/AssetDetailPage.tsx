...
export function AssetDetailPage() {
  ...
  return <AssetDetail key={assetId} />;
}
...
function AssetDetail() {
  ...
  const correctionSnapshot = useRef<{ assetId: string; count: number } | null>(null);
  const correctionWriting = useRef(false);
  const incrementWriting = useRef(false);
  const deleting = useRef(false);
  const blocker = useBlocker(() => correcting || correctionWriting.current);
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => { if (correcting || correctionWriting.current || deleting.current) event.preventDefault(); };
  ...
  function openCorrection() { correctionSnapshot.current = { assetId: asset.id, count: asset.usageCount }; setCorrectionValue(String(asset.usageCount)); setCorrectionError(null); setCorrecting(true); }
  async function submitCorrection(event: FormEvent) {
    event.preventDefault(); if (correctionWriting.current || !correctionSnapshot.current) return;
    correctionWriting.current = true; setCorrectionBusy(true); setCorrectionError(null);
    try { await correctUsageCount(correctionSnapshot.current.assetId, correctionSnapshot.current.count, correctionValue); setCorrecting(false); setUsageFeedback('使用次数已更正'); }
  ...
  return <section data-pwa-busy={busy || incrementing || correctionBusy ? 'true' : undefined}><div className="page-heading"><h1>{asset.name}</h1><Link className="button" to={`/assets/${asset.id}/edit`}>编辑</Link></div>
    ...
    {values.netCostCents < 0 && <p className="notice">收益已超过投入</p>}{values.clockBeforePurchase && <p className="notice">设备日期早于好物日期，请检查系统时钟；计费天数最低按 1 天显示。</p>}
