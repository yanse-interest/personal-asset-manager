...
import type { Asset, CostMode } from '../domain/types';
import { ErrorMessage } from '../components/ErrorMessage';

interface Draft { name: string; purchaseCost: string; purchaseDate: string; costMode: CostMode; initialUsageCount: string; expiryDate: string; note: string }
const emptyDraft = (): Draft => ({ name: '', purchaseCost: '', purchaseDate: localToday(), costMode: 'day', initialUsageCount: '0', expiryDate: '', note: '' });
...
export function AssetFormPage() {
  ...
  useEffect(() => { if (!assetId) return; void getAsset(assetId).then(asset => {
    if (!asset) setMissing(true); else { setSource(asset); setDraft({ name: asset.name, purchaseCost: (asset.purchaseCostCents / 100).toFixed(2), purchaseDate: asset.purchaseDate, costMode: asset.costMode, initialUsageCount: String(asset.usageCount), expiryDate: asset.expiryDate ?? '', note: asset.note ?? '' }); }
    setLoaded(true);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const input = { name: draft.name, purchaseCost: draft.purchaseCost, purchaseDate: draft.purchaseDate, costMode: draft.costMode, initialUsageCount: draft.initialUsageCount, expiryDate: draft.expiryDate || null, note: draft.note };
