...
export function DashboardPage() {
  ...
  return <section className="dashboard-page">
    ...
    <nav className="category-tabs" aria-label="自定义分类账本">
      <button className={categoryId === 'all' ? 'active' : ''} onClick={() => setFilter('category', 'all', 'all')}>全部</button>
      {sortedCategories(categories).map(category => <button key={category.id} className={categoryId === category.id ? 'active' : ''} onClick={() => setFilter('category', category.id, 'all')}>{category.name} <span>{summaries.filter(item => item.asset.categoryId === category.id).length}</span></button>)}
      {summaries.some(item => item.asset.categoryId === null) && <button className={categoryId === 'uncategorized' ? 'active' : ''} onClick={() => setFilter('category', 'uncategorized', 'all')}>未分类 <span>{summaries.filter(item => item.asset.categoryId === null).length}</span></button>}
