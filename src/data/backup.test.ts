...
import type { Asset, BackupV1, BackupV2, Category, CostRecord, LegacyAssetV1, RevenueRecord } from '../domain/types';
...
const asset: Asset = { ...legacyAsset, categoryId, lifecycleStatus: 'sold', endedDate: '2026-09-13' };
...
const sampleV2 = (): BackupV2 => ({ format: 'large-asset-cost-backup', schemaVersion: 2, exportedAt: timestamp, currency: 'CNY', assets: [{ ...asset }], categories: [{ ...category }], costRecords: [{ ...cost }], revenueRecords: [{ ...revenue }] });
...
describe('v1/v2 JSON backup', () => {
  it('validates frozen v1 fields then upgrades in memory without changing history', async () => {
    ...
    expect(imported.assets[0]).toEqual({ ...legacyAsset, categoryId: null, lifecycleStatus: 'active', endedDate: null });
  ...
  it('rejects invalid v2 facts before any write', async () => {
    ...
    await expect(replaceFromBackup({ ...sampleV2(), schemaVersion: 3 }, database, now)).rejects.toThrow('schemaVersion');
    expect(await getDashboardSnapshot(database)).toEqual(previous);
  });
