...
import type { Asset, Category, CostRecord, RevenueRecord } from '../../src/domain/types';
...
async function run() {
  ...
  try {
    if (role === 'setup') {
      ...
      const [assets, categories, costs, revenues] = await Promise.all([database.assets.toArray(), database.categories.toArray(), database.costRecords.toArray(), database.revenueRecords.toArray()]);
      if (role === 'verify' && (JSON.stringify(assets) !== JSON.stringify([asset]) || JSON.stringify(categories) !== JSON.stringify([category]) || JSON.stringify(costs) !== JSON.stringify([cost]) || JSON.stringify(revenues) !== JSON.stringify([revenue]))) {
        throw new Error('下载文件重新导入后，四表未恢复原始全字段数据');
      }
